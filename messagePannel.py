...
class BubbleText(QLabel):
    ...
    def __init__(self,listItem,listView,text = DEFAULT_MSG,lr = True):
        ...
        self.setFont(QFont("Times",10,QFont.Normal))
...
class LabelHead(QLabel):
    ...
    def setPicture(self,addr):
        ...
        self.setPixmap(img.scaled(36,36))
        return True
...
class TextItem(QWidget):
    ...
    def __init__(self, listItem, listView, text = DEFAULT_MSG, lr=True, head = DEFAULT_HEAD):
        ...
        if lr is not True:
            ...
            hbox.addWidget(text)
            hbox.addSpacerItem(QSpacerItem(1,1,QSizePolicy.Expanding,QSizePolicy.Preferred))
        hbox.setContentsMargins(20,20,20,0)
        self.setLayout(hbox)
...
class BubbleImage(QLabel):
    ...
    def __init__(self,listItem,listView,img = DEFAULT_IMG,lr = True,maxWidth = 200):#图片显示的最长宽度
        self.listItem = listItem
        self.listView = listView
        self.img = img
...
class MsgList(QListWidget):
    ...
    def __init__(self):
        ...
        self.setStyleSheet("QListWidget{margin:10px;font-size:12px;border-bottom:1px solid #dddddd;border-top:1px solid #dddddd;padding:0px;background-color:#f5f5f5;}"
            ...
            "QScrollBar::add-line{background:transparent;}")        # 设置所有样式锁定
...
class MessagePannel(QSplitter):
    def closesys(self, event):
        sys.exit()
    def setSendTextMsgEvent(self,SendTextMsgEvent):
        self.msgInput.textEntered.connect(SendTextMsgEvent)
    def setSendImageMsgEvent(self,SendImageMsgEvent):
        self.msgInput.imgEntered.connect(SendImageMsgEvent)
    def showNoneBackground(self):
        self.msgList.hide()
        self.msgInput.hide()
        self.msg_pannel_title_label.setText('')
        self.nonbackground.show()
        self.show()
    def showMessageInOut(self):
        self.msgList.show()
        self.msgInput.show()
        self.nonbackground.hide()
        self.show()
    def  __init__(self,parent = None):
        super(MessagePannel,self).__init__(parent)
        ...
        self.msg_pannel_title_label = QLabel()
        self.msg_pannel_title_label.setGeometry(QRect(28, 18, 400, 18))
        self.msg_pannel_title_label.setStyleSheet("margin:0px;font-size:14px;border:0px;padding:0px;background-color:#f5f5f5;")
        self.msg_pannel_title_label.setParent(self.titlepannel)
        close_label = QLabel()
        ...
        self.msgList.setParent(self)
        self.msgInput = MsgInput()
        self.msgInput.setStyleSheet("padding:25px;border:0px;background-color:#f5f5f5;")

        self.msgInput.setParent(self)
        self.nonbackground=QLabel()
        self.nonbackground.setAlignment(Qt.AlignCenter)
        image=QImage()
        image.load('img/backgroundimg.png')
        self.nonbackground.setPixmap(QPixmap.fromImage(image).scaled(120,100))
        self.nonbackground.setParent(self)
if __name__=='__main__':
        def test(event):
            print event
        app = QApplication(sys.argv)
        menu_pannel = MessagePannel()
        
        menu_pannel.showMessageInOut()
        sys.exit(app.exec_())